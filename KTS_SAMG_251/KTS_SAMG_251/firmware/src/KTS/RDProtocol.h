 ...
 *                 La seguente seriale ÃƒÂ¨ dedicata per la comunicazione tra KTFS e RD2.0
 *
 *  Esempio:
 *        
 *************************************************************************************/

#ifndef __RDProtocol_H
#define __RDProtocol_H
...
#define COMMAND_DATA_DEBUG        'D'    // comando per la lettura dei dati per il debug, misure velocitÃƒ  motori, etc... 
...
enum index_Head_Message_Ser1{
   ...
   IHM1_POS_CRC_LO,               // byte_2: posizione cksum_16 (byte LO) che ÃƒÂ¨ anche il penultimo byte
   IHM1_TYPE_COMAND,              // byte_3: Tipo di comando/richiesta
   IHM1_START_DATA                // byte_4: Inizio BYTE Dati, lo sviluppo dei dati ÃƒÂ¨ in funzione del TYPE_COMAND
};
  ...
  // PAGINA 3: Pressione
   ...
   IRSD_LEV_LINK_COOLER,                   // 0/10 Level
	...
	1 byte STATUS:	bit.4: 1=IN_ALLARME, 0=NO ALLARMI
					bit.3: [Comando da UnitÃ (1 ACCENDITI/SPEGNITI)]
					bit.2: ON=1/OFF=0
					bit.1: AIR=1/WATER=0
					bit.0: CONNECT
	* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
   ...
   IRSD_MEASURE_TW_HEAT_LO,                // 2 byte:  Temperature water se  l'accessory Ã¨ HWD altrimenti dato inutile
   ...
   IRSD_MEASURE_TW_PREHEAT_LO,             // 2 byte:  Temperature water se  l'accessory è PHWD 
...
enum index_message_resp_polling_base {
  ...
  // dati utili a capire se vi ÃƒÂ¨ stata fatta una variazione di eeprom da parte di altri. 
