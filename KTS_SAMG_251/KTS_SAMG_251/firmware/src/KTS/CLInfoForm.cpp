...
void CLInfoForm_ResponseCallback( byte* rxBuffer, void* userData )
{
	CLInfoForm*	InfoDataForm	= (CLInfoForm*) userData;

	if (InfoDataForm == NULL)
		return;
   
    InfoDataForm->Measure_CO2_1 = ((word) MAKEWORD( rxBuffer, IRSD_MEASUR_CO2_1_HI, IRSD_MEASUR_CO2_1_LO ));
    InfoDataForm->Measure_VOC_1 = ((word) MAKEWORD( rxBuffer, IRSD_MEASUR_VOC_1_HI, IRSD_MEASUR_VOC_1_LO ));
    InfoDataForm->Measure_RH_1 = rxBuffer[ IRSD_MEASUR_RH_1 ];
    InfoDataForm->Measure_CO2_2 = ((word) MAKEWORD( rxBuffer, IRSD_MEASUR_CO2_2_HI, IRSD_MEASUR_CO2_2_LO ));
    InfoDataForm->Measure_RH_2 = rxBuffer[ IRSD_MEASUR_RH_2 ];
    InfoDataForm->Firm_Ver_CO2_1 = rxBuffer[ IRSD_FIRM_VER_CO2_1 ];
    InfoDataForm->Firm_Ver_CO2_2 = rxBuffer[ IRSD_FIRM_VER_CO2_2 ];
    InfoDataForm->CO2_1_LinkLevel = rxBuffer[ IRSD_LEV_LINK_CO2_1 ];
    InfoDataForm->CO2_2_LinkLevel = rxBuffer[ IRSD_LEV_LINK_CO2_2 ];
    InfoDataForm->Measure_IAQ_1 = ((word) MAKEWORD( rxBuffer, IRSD_MEASUR_IAQ_1_HI, IRSD_MEASUR_IAQ_1_LO )) & 0x0FFF;
    InfoDataForm->Measure_IAQ_2 = ((word) MAKEWORD( rxBuffer, IRSD_MEASUR_IAQ_2_HI, IRSD_MEASUR_IAQ_2_LO )) & 0x0FFF;
    InfoDataForm->IAQ_1_Accuracy = (rxBuffer[ IRSD_MEASUR_IAQ_1_HI ] >> 4) & 3;
    InfoDataForm->IAQ_2_Accuracy = (rxBuffer[ IRSD_MEASUR_IAQ_2_HI ] >> 4) & 3;
    InfoDataForm->CO2_1_Status = rxBuffer[ IRSD_STATUS_CO2_1 ];
    InfoDataForm->CO2_2_Status = rxBuffer[ IRSD_STATUS_CO2_2 ];
    InfoDataForm->CO2_1_Accuracy = rxBuffer[ IRSD_ACCURACY_CO2_1 ] & 3;
    InfoDataForm->CO2_2_Accuracy = rxBuffer[ IRSD_ACCURACY_CO2_2 ] & 3;
}
...
CLFormResult CLInfoForm::Open()
{
	...
	while (true)
	{
			...
			millis() < previousTimerRefresh)
		{
			...
			if (dataDebugRequestQueueIndex == -1)
			{
				dataDebugRequestQueueIndex	= ComQueue_AddDataDebugRequest( &dataDebugRequestStatus, CLInfoForm_ResponseCallback );
				if (dataDebugRequestQueueIndex != -1)
...
// CO2_1 VALUE            
            ...
            strcat ( text1 , text ); strcat ( text1 , "|" );
            memset ( text, 0, sizeof(text));
            sprintf( text, "%4d", Measure_CO2_1 );
            strcat ( text1 , text ); strcat ( text1 , "|" );
            memset ( text, 0, sizeof(text));
            sprintf( text, "%4d", Measure_VOC_1 );
            strcat ( text1 , text ); strcat ( text1 , "|" );
            memset ( text, 0, sizeof(text));
