...
void CLMainForm::SetRunningMode( CLRunningMode value )
{
	...
	switch (m_RunningMode)
	{
		...
		case CLRunningMode_Stepless:
			m_Stepless_CurrentSpeed	= SpeedMode_Stepless_GetValue( VentilationControl_Get() );
...
void CLMainForm::RunningMode_Process()
{
	...
	else
	{
		...
		{
			case CLRunningMode_Stepless:
				if (m_Stepless_CurrentSpeed != SpeedMode_Stepless_GetValue( VentilationControl_Get() ))
				{
					m_Stepless_CurrentSpeed	= SpeedMode_Stepless_GetValue( VentilationControl_Get() );
					m_Stepless_Value		= m_Stepless_CurrentSpeed;
					refreshDynamicObject	= true;
...
void CLMainForm::Stepless_WriteValue_Process()
{
	...
	if (m_Stepless_WriteValue_InProgress)
	{
		...
		{
			SpeedMode_Stepless_SetValue( VentilationControl_Get(), m_Stepless_WriteValue_SavedValue );
		...
		millis() < m_Stepless_WriteValue_TimerMilliseconds)
	{
		...
		if (m_Stepless_Value != SpeedMode_Stepless_GetValue( VentilationControl_Get() ))
		{
			// Salva il valore corrente
			m_Stepless_WriteValue_SavedValue	= SpeedMode_Stepless_GetValue( VentilationControl_Get() );

			// Imposta il nuovo valore
			if (!SpeedMode_Stepless_SetValue( VentilationControl_Get(), m_Stepless_Value ))
			{
				Stepless_SetValue( m_Stepless_WriteValue_SavedValue );
			}
			else
